		...
		while (m_running)
			try
			{
				...
				for (unsigned i = 0; !completed; ++i)
				{
					...
					try
					{
						prpc->etrue_submitHashrate(toJS(rate), "0x" + id.hex());
					}
					...
					Json::Value v = prpc->etrue_getWork();
				...
				if (EthashAux::eval(current.seedHash, current.headerHash, solution.nonce).value < current.boundary)
				{
					bool ok = prpc->etrue_submitWork("0x" + toString(solution.nonce), "0x" + toString(current.headerHash), "0x" + toString(solution.mixHash));
					if (ok) {
				...
				{
					bool ok = prpc->etrue_submitWork("0x" + toString(solution.nonce), "0x" + toString(previous.headerHash), "0x" + toString(solution.mixHash));
					if (ok) {
						cnote << "B-) Submitted and accepted.";
